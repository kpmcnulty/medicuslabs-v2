...
async def merge_diseases(disease_id: int, target_disease_id: int) -> Dict[str, Any]:
    ...
    async with get_pg_connection() as conn:
        ...
        source = await conn.fetchrow("SELECT * FROM diseases WHERE id = $1", disease_id)
        target = await conn.fetchrow("SELECT * FROM diseases WHERE id = $1", target_disease_id)
        ...
        async with conn.transaction():
            ...
            # Merge synonyms and search terms
            await conn.execute("""
                UPDATE diseases
                SET synonyms = array_cat(
                    COALESCE(synonyms, ARRAY[]::text[]), 
                    array_append($2::text[], $3)
                ),
                search_terms = array_cat(
                    COALESCE(search_terms, ARRAY[]::text[]),
                    $4::text[]
                )
                WHERE id = $1
            """, target_disease_id, source['synonyms'] or [], source['name'], source['search_terms'] or [])
